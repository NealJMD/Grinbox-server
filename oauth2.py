...
HARDCODED_REDIRECT_URI = 'urn:ietf:wg:oauth:2.0:oob'
...
def GeneratePermissionUrl(client_id, scope='https://mail.google.com/', redirect_uri=HARDCODED_REDIRECT_URI, access_type='online', state=''):
  ...
  if len(state) > 0: params['state'] = state
  return '%s?%s' % (AccountsUrl('o/oauth2/auth'),
                    FormatUrlParams(params))


def AuthorizeTokens(client_id, client_secret, authorization_code, redirect_uri=HARDCODED_REDIRECT_URI):
